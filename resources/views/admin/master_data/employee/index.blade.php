<x-app-layout>
    <div x-data="employeeModal()" class="p-8 bg-gray-100 min-h-screen">
        <div class="bg-white rounded-2xl shadow-md p-6 max-w-5xl mx-auto">
            <form method="POST" action="{{ route('admin.master_data.employee.store') }}">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end mb-6">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">
                            Employee ID <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="emp_id"
                            class="w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 px-3 py-2"
                            placeholder="Enter employee ID" required />
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">
                            Employee Name <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="emp_name"
                            class="w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 px-3 py-2"
                            placeholder="Enter employee name" required />
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">
                            Department <span class="text-red-500">*</span>
                        </label>
                        <select name="dept_id" id=""
                            class="w-full border truncate border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 px-3 py-2"
                            required>
                                <option value="" disabled {{ old('dept_id') ? '' : 'selected' }}>
                                    Select a Department
                                </option>
                            @foreach ($departements as $dept)
                                <option value="{{ $dept->id }}" {{ old('dept_id') == $dept->id ? 'selected' : '' }}>
                                    {{ $dept->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">
                            Position <span class="text-red-500">*</span>
                        </label>
                        <select name="pos_id" id=""
                            class="w-full border truncate border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 px-3 py-2"
                            placeholder="Manager" required>
                                <option value="" disabled {{ old('pos_id') ? '' : 'selected' }}>
                                    Select a Position
                                </option>
                            @foreach ($positions as $pos)
                                <option value="{{ $pos->id }}" {{ old('pos_id') == $pos->id ? 'selected' : '' }}>
                                    {{ $pos->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="md:col-span-4 flex justify-end mt-2">
                        <button type="submit"
                            class="inline-flex items-center bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2 rounded-md transition duration-200">
                            <i class="bi bi-plus-lg mr-2"></i> Add Employee
                        </button>
                    </div>
                </div>
            </form>
            @php
    $alertClasses = [
        'success' => 'bg-green-100 border border-green-400 text-green-700',
        'error' => 'bg-red-100 border border-red-400 text-red-700',
        'warning' => 'bg-yellow-100 border border-yellow-400 text-yellow-700',
        'info' => 'bg-blue-100 border border-blue-400 text-blue-700',
    ];
@endphp

@foreach (['success', 'error', 'warning', 'info'] as $msg)
    @if (session($msg))
        <div 
            x-data="{ showAlert: true }"
            x-show="showAlert"
            x-init="setTimeout(() => showAlert = false, 3000)"
            x-transition
            class="{{ $alertClasses[$msg] }} px-4 py-3 rounded relative mb-4 mt-2"
            role="alert">
            <strong class="font-bold mr-2">{{ ucfirst($msg) }}:</strong>
            <span class="block sm:inline">{{ session($msg) }}</span>

            <!-- Tombol Close -->
            <span @click="showAlert = false" 
                class="absolute top-0 bottom-0 right-0 px-4 py-3 cursor-pointer">
                <svg class="fill-current h-6 w-6 
                    text-{{ $msg === 'error' ? 'red' : ($msg === 'success' ? 'green' : 'gray') }}-700" 
                    role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                    <title>Close</title>
                    <path d="M14.348 5.652a.5.5 0 0 0-.707 0L10 9.293 6.36 5.652a.5.5 0 1 0-.707.707L9.293 10l-3.64 3.64a.5.5 0 0 0 .707.707L10 10.707l3.64 3.64a.5.5 0 0 0 .707-.707L10.707 10l3.64-3.64a.5.5 0 0 0 0-.708z"/>
                </svg>
            </span>
        </div>
    @endif
@endforeach
            <div class="overflow-x-auto">
                <table class="min-w-full border border-gray-200 rounded-lg overflow-hidden">
                    <thead class="bg-gray-100 text-gray-700 text-sm uppercase">
                        <tr>
                            <th class="px-4 py-3 text-center">No.</th>
                            <th class="px-4 py-3 text-center">Employee Name</th>
                            <th class="px-4 py-3 text-center">Department</th>
                            <th class="px-4 py-3 text-center">Position</th>
                            <th class="px-4 py-3 text-center">Action</th>
                        </tr>
                    </thead>

                    <tbody class="text-gray-800">
                        <span hidden>{{ $count = 1 }}</span>
                        @foreach ($employees as $emp)
                            <tr class="border-t hover:bg-gray-50">
                                <td class="px-4 py-3 text-center font-medium">{{ $count }}</td>
                                <td class="px-4 py-3 text-center">{{ $emp['name'] }}</td>
                                <td class="px-4 py-3 text-center">{{ $emp->departement->name }}</td>
                                <td class="px-4 py-3 text-center">{{ $emp->position->name }}</td>
                                <td class="px-4 py-3 text-center">
                                    <button @click="editEmployee('{{ $emp->id }}', {
                                    id_card: '{{ $emp->id_card }}',
                                    name: '{{ $emp->name }}',
                                    dept_id: '{{ $emp->departement->id }}',
                                    pos_id: '{{ $emp->position->id }}'
                                    })" class="text-blue-600 hover:underline mr-3">
                                        Edit
                                    </button>

                                    <button @click="deleteEmployee('{{ $emp->id_card }}')"
                                        class="text-red-600 hover:underline">
                                        Delete
                                    </button>
                                </td>
                            </tr>
                        <span hidden>{{ $count++ }}</span>
                        @endforeach
                    </tbody>
                </table>
                
            </div>

        </div>

        <div x-show="openModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
            x-transition>
            <div class="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 relative">
                <h2 class="text-lg font-semibold text-gray-800 mb-4">Edit Employee</h2>
                <form method="POST" :action="updateUrl" id="editEmployeeForm">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Employee Code *</label>
                        <input type="text" name="emp_id" x-model="selected.id_card"
                            class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-green-500 focus:border-green-500" placeholder="Enter new employee ID" required>
                    </div>
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Employee Name *</label>
                        <input type="text" name="emp_name" x-model="selected.name"
                            class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-green-500 focus:border-green-500" placeholder="Enter new employee name" required>
                    </div>
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Employee Department *</label>
                        <select name="dept_id" x-model="selected.dept_id"
                            class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-green-500 focus:border-green-500"
                            required>
                            @foreach ($departements as $dept)
                                <option value="{{ $dept->id }}">{{ $dept->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Employee Position *</label>
                        <select name="pos_id" x-model="selected.pos_id"
                            class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-green-500 focus:border-green-500"
                            required>
                            @foreach ($positions as $pos)
                                <option value="{{ $pos->id }}">{{ $pos->name }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="flex justify-end">
                        <button type="submit"
                            class="bg-green-500 hover:bg-green-600 text-white font-semibold px-6 py-2 rounded-md">
                            Save
                        </button>
                    </div>
                </form>

                <button @click="openModal = false"
                    class="absolute top-3 right-4 text-gray-400 hover:text-gray-600 text-xl">
                    &times;
                </button>
            </div>
        </div>

    </div>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/font/bootstrap-icons.css">
    <script>
        function employeeModal() {
            return {
                openModal: false,
                updateUrl: '',

                selected: {
                    id_card: '',
                    name: '',
                    dept_id: '',
                    pos_id: ''
                },

                editEmployee(employeeId, emp) {
                    this.selected = {
                        id_card: emp.id_card,
                        name: emp.name,
                        dept_id: emp.dept_id,
                        pos_id: emp.pos_id
                    };

                    this.updateUrl = "{{ route('admin.master_data.employee.update', ':id') }}"
                        .replace(':id', employeeId);

                    this.openModal = true;
                },

                async deleteEmployee(employeeId) {
                    try {
                        const response = await fetch(`/admin/master_data/employee/${employeeId}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Accept': 'application/json'
                            }
                        });

                        if (response.ok) {
                            window.location.reload();
                        } else {
                            this.showNotification('Failed to delete employee', 'error');
                        }
                    } catch (error) {
                        console.error('Delete error:', error);
                        this.showNotification('An error occurred', 'error');
                    }
                }
            }
        }
    </script>
</x-app-layout>
